...
def linear_q_learning(theta, current_state_vector, action_index, object_index,
    ...
    y = reward + (1-terminal) * GAMMA * (theta @ next_state_vector).max()
    g = (y - (theta @ current_state_vector)[tuple2index(action_index, object_index)]) * current_state_vector

    theta[tuple2index(action_index, object_index)] += ALPHA * g
# pragma: coderesponse end
